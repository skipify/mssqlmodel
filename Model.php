...
class Model
{
    ...
    public function init(array $setting)
    {
        if (empty($this->vitex->pdo)) {
            $this->vitex->runMiddleware(new Pdo($setting));
            $this->DB = $this->vitex->pdo;
            $this->pdo = $this->DB->pdo;
        }
        return $this;
    ...
    private function buildSql($iscount = false)
    {
        ...
        if ($this->_sql['limit'] && !$iscount) {
            if (!$this->_sql['offset']) {
                $sql .= "top " . $this->_sql['limit'] . ' ';
            } else {
                $sql .= "top " . $this->_sql['limit'] . ' ';
                $xsql =  "select top ". $this->_sql['offset']." ".$this->pk;
                $xsql .= " from " . $this->getTable() . ' ';
                $xwhere = $this->buildWhere();
                if(trim($xwhere)){
                    $xsql .=  $xwhere;
                }
                if ($this->_sql['order']) {
